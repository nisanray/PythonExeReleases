...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.start_btn = AnimatedButton("Start", "#3498db", "#2980b9", "#1f618d")
        ...
        self.stop_btn = AnimatedButton("Stop", "#e67e22", "#d35400", "#a04000")
    ...
    def init_menu_bar(self):
        ...
        # Save Action configuration (with Ctrl+S Shortcut)
        save_action = file_menu.addAction("Save")
        save_action.setShortcut(QKeySequence.StandardKey.Save) # Ensures Ctrl+S globally
    ...
    def update_window_title(self):
        name = os.path.basename(self.filename) if self.filename else "untitled_sequence.json"
    ...
    def file_open(self):
        try:
            ...
            if fname:
                ...
                if isinstance(data, list):
                    # Legacy Format Fallback (pure array of steps)
                    self.steps = [Step.from_dict(s) for s in data]
                    # Reset UI elements to defaults since they weren't in the file
                    self.start_delay_spin.setValue(0)
                    self.reverse_checkbox.setChecked(False)
                    self.loop_times_spin.setValue(0)
                    self.lock_after_spin.setValue(0)
                elif isinstance(data, dict):
                    # Modern Dictionary Format (settings + steps)
                    settings = data.get("settings", {})
                    self.start_delay_spin.setValue(settings.get("start_delay", 0))
                    self.reverse_checkbox.setChecked(settings.get("reverse", False))
                    self.loop_times_spin.setValue(settings.get("loop_times", 0))
    ...
    def start_sequence(self):
        ...
        self.stop_btn.start_pulse_animation("#f39c12")
    ...
    def run_sequence(self):
        ...
        try:
            ...
            run_log_filename = os.path.join("run_logs", f"run_{time.strftime('%Y%m%d_%H%M%S')}.log")
            run_logger = logging.getLogger(f"Run_{time.time()}")
            run_logger.setLevel(logging.INFO)
            run_log_handler = logging.FileHandler(run_log_filename, encoding='utf-8')
            run_log_handler.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] - %(message)s"))
            run_logger.addHandler(run_log_handler)
            run_logger.info("--- Starting Automation Run ---")
            run_logger.info(f"Sequence File: {self.filename or 'Untitled'}")
        except Exception as e:
            logging.error(f"Failed to create run log file: {e}")
        # --------------------------------------
        ...
        try:
            ...
            if run_logger: 
                run_logger.info(f"Configuration -> Start Delay: {delay}s, Loops: {loop_times}, Reverse: {reverse}, Lock After: {lock_delay}s")
            ...
            if delay > 0:
                for i in range(delay, 0, -1):
                    if not self.running or self.abort_flag.is_set(): 
                        if run_logger: run_logger.warning("Run aborted during start delay.")
                        return
                    self.status_update.emit(f"Starting in {i}s...")
                    if run_logger: run_logger.info(f"Waiting for start delay: {i}s remaining...")
                    time.sleep(1)
            ...
            while self.running and not self.abort_flag.is_set():
                try:
                    ...
                    if not current_steps: 
                        if run_logger: run_logger.warning("No steps to execute. Ending run.")
                        break
                    if reverse: current_steps = current_steps[::-1]
                    
                    if run_logger: run_logger.info(f"--- Starting Loop {loops_done + 1} ---")
                    
                    for i, step in enumerate(current_steps):
                        if not self.running or self.abort_flag.is_set(): 
                            if run_logger: run_logger.warning(f"Run aborted at Step {i+1} during Loop {loops_done + 1}.")
                            return
                        ...
                        if run_logger:
                            run_logger.info(f"Executing Step {step_num}/{len(current_steps)}: Action='{step.action}', Params={step.params}, Note='{step.note}'")
                        
                        try:
                            self.execute_step(step)
                            total_steps_executed += 1
                            if run_logger: run_logger.info(f"Step {step_num} executed successfully. Applying post-step delay of {step.delay}s.")
                            
                            if step.delay > 0:
                                if not self.interruptible_sleep(step.delay): 
                                    if run_logger: run_logger.warning(f"Run aborted during delay of Step {step_num}.")
                                    return
                        except Exception as step_error:
                            logging.error(f"Failed to execute step {step_num}: {step_error}")
                            if run_logger: run_logger.error(f"Failed to execute step {step_num}: {step_error}")
                            continue
                    
                    loops_done += 1
                    if loop_times > 0 and loops_done >= loop_times: 
                        if run_logger: run_logger.info(f"Reached configured loop limit ({loop_times}). Finishing sequence.")
                        break
                        
                except Exception as loop_error:
                    logging.error(f"Error in execution loop {loops_done + 1}: {loop_error}")
                    if run_logger: run_logger.error(f"Error in execution loop {loops_done + 1}: {loop_error}")
                    loops_done += 1
                    if loops_done >= 100: break
            
            self.status_update.emit("Done.")
            if run_logger: run_logger.info(f"Sequence execution finished. Total steps executed: {total_steps_executed}")
            
            if lock_delay > 0 and self.running:
                try:
                    self.status_update.emit(f"Locking in {lock_delay}s...")
                    if run_logger: run_logger.info(f"Initiating workstation lock countdown: {lock_delay}s...")
                    if self.interruptible_sleep(lock_delay): 
                        if run_logger: run_logger.info("Locking workstation now.")
                        lock_workstation()
                    else:
                        if run_logger: run_logger.warning("Workstation lock aborted.")
                except Exception as lock_error:
                    logging.error(f"Error during workstation lock: {lock_error}")
                    if run_logger: run_logger.error(f"Error during workstation lock: {lock_error}")
                    
        except Exception as e:
            logging.error(f"Critical error in sequence execution: {e}")
            if run_logger: run_logger.critical(f"Critical error in sequence execution: {e}")
            ...
            try:
                ...
                QTimer.singleShot(0, self._reset_ui_state)
                # --- Cleanup Run Logger ---
                if run_logger and run_log_handler:
                    run_logger.info("--- Automation Run Log Closed ---")
    ...
    def execute_step(self, step: Step):
        ...
        try:
            ...
            raise  # Re-raise so the sequence runner can catch and log it appropriately
        finally:
            try:
